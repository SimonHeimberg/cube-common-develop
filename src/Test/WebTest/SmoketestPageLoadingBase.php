...
class SmoketestPageLoadingBase extends WebTestBase
{
    ...
    protected function loadPage($method, $path, $info)
    {
        ...
        switch ($code) {
            case Response::HTTP_OK:
                ...
                break;
            case Request::HTTP_NOT_FOUND:
            case Request::HTTP_INTERNAL_SERVER_ERROR:
    ...
    public function testSimplePageLoadingWithParameterUrl($method, $url, $info)
    {
        ...
        $this->throwIfException($aw);
        if ($aw['code'] == Request::HTTP_NOT_FOUND && (strpos($aw['msg'], 'entity') || strpos($aw['msg'], ' not found')) ||
            $aw['code'] == Request::HTTP_INTERNAL_SERVER_ERROR && strpos($aw['msg'], 'The file "') && strpos($aw['msg'], '" does not exist')
        ) {
